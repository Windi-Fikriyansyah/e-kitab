...
@section('content')
    <div class="page-heading">
        <h2>{{ isset($PS) ? 'Edit Penerimaan Stok' : 'Tambah Penerimaan Stok' }}</h2>
    </div>
    <div class="page-content">
        ...
        <div class="card">
            <div class="card-body">
                <form id="productForm" action="{{ isset($PS) ? route('kelola_data.PenerimaanStok.update', $PS->id) : route('kelola_data.PenerimaanStok.store') }}" method="POST">
                    @csrf
                    @if(isset($PS))
                        @method('POST')
                    @endif
                    ...
                    <div class="form-group">
                        <label for="tambah_stock">Penerimaan Stok</label>
                        <input type="number" name="tambah_stock" id="tambah_stock" class="form-control" value="{{ old('tambah_stock', $PS->tambah_stock ?? '') }}">
                    </div>


                    <button type="submit" class="btn btn-primary">{{ isset($PS) ? 'Update' : 'Tambah' }}</button>
                    <a href="{{ route('kelola_data.PenerimaanStok.index') }}" class="btn btn-warning">Kembali</a>
                </form>
            </div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#barcode').select2({
                ...
                ajax: {
                    url: '{{ route('kelola_data.PenerimaanStok.search') }}',
                    dataType: 'json',
                    delay: 250,
        ...
        var url = '{{ route("kelola_data.PenerimaanStok.getByBarcode", ":barcode") }}';
